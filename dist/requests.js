...
let cookies = {};
const interestingCookies = ['ASP.NET_SessionId', 'LegendOnlineAffinity', 'Responsive', 'APP_LGD_COOKIE_TEST'];
const getCookies = (notFormattedCookies) => {
  let tmp = notFormattedCookies
    .map(c => c.split(';'))
    .map(c =>
      c.filter(nestedC =>
        !!interestingCookies.find(i => nestedC.includes(i))
      )
    );
    ...
    return new Promise((res, rej) => {
      request.get({
        ...
        if (!err && parsedBody.Success) {
          console.log(`Class ${lesson.className} at ${lesson.time} added to basket`);
          return res(body);
        }
  ...
  completeBasket() {
    return new Promise((res, rej) => {
      request.get({
        ...
        headers: {
          'Cookie': cookies,
          'Referer': 'https://gymbox.legendonlineservices.co.uk/enterprise/Basket/',
        ...
        console.error(`Payment rejected with status code: ${_.statusCode}, req: ${_}`);
        return rej(err);
      });
    });
  },
  confirmPayment() {
    return new Promise((res, rej) => {
      request.get({
        ...
        console.error(`Payment confirmation rejected with status code: ${_.statusCode}`);
        return rej(err);
      });
    });
  }
